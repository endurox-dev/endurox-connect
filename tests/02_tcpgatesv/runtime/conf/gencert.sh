...
echo "subjectAltName=DNS:$TEST_HOSTNAME" > subj.ext
...
# Sign server cert
openssl x509 -req -in server.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out server.crt -extfile subj.ext
...
# Sign client cert
openssl x509 -req -in client.csr -CA ca.crt -CAkey ca.key -CAserial ca.srl -out client.crt -extfile subj.ext
