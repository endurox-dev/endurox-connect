...
"%s" prepresenting the error message. This error string is used in case if
'errors' parameter is set to *text*. The format string will be used only in case
of tpcall error. If no error occurs, then service answer message is returned.
If case if asyncrhouns invocation is done to service ('async' param set to *1*),
...
Json error message format string. Normally this would be set to json field like
