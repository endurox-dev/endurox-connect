...
in the same connection thread. Thus ensuring the sequence of the messages to be
according to the message sequence in socket. The default is *0* - disabled,
meaning that incoming messages are processed in out of order sequence (processed
by multiple threads).
...
are processed in out of order sequence.
